...
void DBExecuter::deleteData(std::string table, std::vector<std::string> columns, std::vector<std::string> condition) {
	database_->connectToDatabase();
	std::string columns_as_string_ = boost::algorithm::join(columns, ", ");
	std::string conditions_as_string_ = boost::algorithm::join(condition, ", ");

	std::string statement = "DELETE FROM " + table + " WHERE (" + columns_as_string_ + ") = (" + conditions_as_string_ + ")";
